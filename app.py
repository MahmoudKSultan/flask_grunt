...
# USED
def process_pdf_plumber(file_name) -> str:
    """Process PDF with pdfplumber first, falling back to improved OCR if needed"""
    try:
        with pdfplumber.open(file_name) as pdf:
            text = ''
            # Extract text from each page and store it as a string
            for page in pdf.pages:
                text += page.extract_text()
        return text
    except Exception as e:
        logger.error(f"Error in PDF processing with pdf plumber: {str(e)}. Switching to the old function")
        return process_pdf_with_ocr(file)
...
# USED
def parse_line_items(text) -> dict:
    """Parse line items"""
    chunks = []
    # Supports new additions of other splits
    splits_texts = ("Recap by Category with Depreciation","Recap by Room",)
    for t in splits_texts:
        # Output is a list so new splits can be made
        text,splitted_text = text.split(t)
        chunks.insert(0,splitted_text)

    pattern = re.compile(r"""
        ([A-Za-z\s]+)      # Match one or more letters or spaces (name)
        \s+                
        ([\d,]+\.\d{2})    # Match RCV
        \s*
        ([\d,]+\.\d{2})?   # Match Depreciation if existant
        \s*
        ([\d,]+\.\d{2})    # Match the ACV
    """, re.VERBOSE)
    for i,text in enumerate(chunks):

        # Inferior than 10 string lengths do not hold any meaning
        # Turns every text in chunk to a list of strings
        chunks[i] = tuple(filter(lambda x: len(x) > 10,text.split('\n')))

        chunks[i] = (j.lower() for j in chunks[i] \
        if 'page' not in j.lower() and 'coverage' not in j.lower())

        ##A
        chunks[i] = list(pattern.match(j).groups() for j in chunks[i] if pattern.match(j))

    return dict(zip(splits_texts,chunks))
...
# TO BE USED
def compare_line_items(items1: dict, items2: dict):
    ...
    try:
        ...
        # item[1] refers to the RCV whilst item[0] to the name
        dict1 = {}
        for categories in items1:
            for item in items1[categories]:
                dict1[item[0]] = item[1]

        dict2 = {}
        for categories in items2:
            for item in items2[categories]:
                dict2[item[0]] = item[1]

        names1,names2 = set(dict1.keys()),set(dict2.keys())
        common = names1 & names2
        comparison_results['unique_to_doc1'] = tuple(sorted(list(names1 - names2)))
        comparison_results['unique_to_doc2'] = tuple(sorted(list(names2 - names1)))

        # Creates a table of values
        # name | cost1 | cost2 | cost1 - cost2
        # if cost exists
        dataframe = []
        atof = lambda x: float(x.replace(',',''))
        for word in sorted(list(names1 | names2)):
            if word in common:
                dataframe.append((word,a:=atof(dict1[word]),b:=atof(dict2[word]),round(a-b,2)))
            elif word in comparison_results['unique_to_doc1']:
                dataframe.append((word,atof(dict1[word]),None,None))
            elif word in comparison_results['unique_to_doc2']:
                dataframe.append((word,None,atof(dict2[word]),None))
        comparison_results['dataframe'] = dataframe
...
def compare_pdfs():
    ...
    try:
        ...
        doc1_text = process_pdf_plumber(temp_file1.name)
        doc2_text = process_pdf_plumber(temp_file2.name)
        ...
        # Define the regex pattern to match the value after "Coverage: Dwelling"
        # Define the regex pattern to find the number after "APPLIANCES"
        pattern = r"APPLIANCES\s+([\d,]+\.\d{2})"

        # Search for the pattern in the text
        match = re.search(pattern, doc1_text)
        ...
        if match:
            ...
            categorized_items['APPLIANCES']['items'][0]['doc1_cost'] = numeric_value

        else:
            print("Value not found")

        # categorized_items['APPLIANCES']['items'][0]['doc1_occurrences']['total_rcv'] = numeric_value
...
def compare_pdfs_useless(doc1_text, doc2_text):
